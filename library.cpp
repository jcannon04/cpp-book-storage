...
void library::find_book(string search_title)
{
  ...
  for (it = books.begin(); it != books.end(); it++)
  {
    if (it->title.find(search_title) != std::string::npos)
    { // if search_title is a substring
      print.push_back(*it);
    }
  }
...
void library::delete_book(string delete_book)
{
  ...
  else
  {
    ...
    { // delete by isbn
      ...
      if (it->title.compare(delete_book) == 0)
      { // delete by title
        it = books.erase(it);
        it--;
      }
